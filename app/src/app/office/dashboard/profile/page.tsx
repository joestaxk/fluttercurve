...
function Page({state}:{state: userDataStateType}) {
  ...
  async function handlePasswordUpdate(ev:any) {
    ...
    try {
      const res = await auth.updatePassword(cookies['x-access-token'], {oldPassword: data.oldPassword,  newPassword: data.newPassword});
      console.log(res)
        ...
        <div className="bg-white lg:w-[83%] w-full shadow rounded-lg min-h-[150px] md:p-4 p-1 flex items-center md:gap-0 gap-4 justify-between">
          ...
          <div className="w-1/2">
            <div className="flex flex-col justify-end">
              ...
              <button onClick={() => copyToClipboard(`http://localhost:3001/register?ref=${state.userName}`)} className="flex w-fit bg-[#212121] p-3 mt-3 gap-2 transition hover:shadow-2xl shadow-[#212121dd] duration-200 text-white rounded-md">
        ...
        <div className="xl:w-[40%] n:w-[65%] md:w-[80%] w-full min-h-[300px] p-5 rounded-md border-[1px] border-[#cccc] bg-white">
          <form action="" onSubmit={handlePersonalUpdate}>
            ...
            <div className="w-full">
              <button  type="submit" className="bg-[#242424] transition-all duration-500 disabled:bg-[#242424cf] rounded-md flex gap-2 p-4 text-[#fff] font-bold" disabled={loading.loadingForm1}>
                {
