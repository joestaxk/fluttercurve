 ...
 */
handleCompoundingServices.successfulCompoundingCharge = async function (
  chargeID: string
) {
  const res: any = await compoundingDeposit.findOne({ where: { chargeID } });
  if (!res?.clientId) return;

  const { uuid, id, userName, email, fullName, ipAddress }: any =
  ...
  await compoundingDeposit.update(
    { status: "SUCCESSFUL" },
    { where: { chargeID } }
  );
...
handleCompoundingServices.updateEarning = async function ({
  clientId,
  chargeID,
  plan,
  duration,
    ...
    compoundingDeposit.update(
      {
        ...
        investmentCompleted:
          parseInt(duration) === remainingDays ? true : false,
...
function calculateEffectiveInterestRate(
  ...
  return (
    (1 + nominalInterestRate / compoundingPeriods) ** compoundingPeriods - 1
  );
}
...
function calculateFutureValue(
  ...
  return initialAmt * (1 + EIR) ** compoundingPeriods;
}

function TotalLogic(
  initialAmt: any,
  getDecimalInterest: any,
  ...
  const getFutureVal = calculateFutureValue(
    initialAmt,
