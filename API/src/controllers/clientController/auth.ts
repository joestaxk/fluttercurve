...
authController.register = async function(req, res, next) {
    ...
    try {
        ...
        const template = `
           ...
           <a href="${config.APP_URI}/verification/${createData.uuid}">
...
authController.login = async function(req:any, res:any, next) {
    ...
    try {
        const ifExist:ClientInterface<string> = await Client.findOne({where: {email}, include:['Referrals', 'userAccount', 'userCompounding']}) as any;
        if (!ifExist) {
        ...
        res.status(httpStatus.OK).send({ message: "You've signed in successfully", userData: {
            ...
            userCompounding: ifExist.userCompounding
        }, session: access})
    } catch (error) {
        console.log(error)
        res.status(httpStatus.BAD_REQUEST).send(error)
...
authController.forgetPassword = async function(req, res, next) {
    ...
    try {
        ...
        const template = `
           ...
           <a href="${config.APP_URI}/forget-password/${keyToken}" style="text-decoration: none">
