...
const profile_data: string | void | undefined =
  helpers.getLocalItemStr("profile_data");

export default function AdminDashboardLayout() {
  const [cookies] = useCookies();
  const [appContextData, setAuth] = useState({
    userData,
    userSession: cookies["xat"],
  });
  const [profileDataContext, setProfileData] = useState<string | any>(
    profile_data
  );
      ...
      <section>
        ...
        >
          <AuthContext.Provider
            value={{ appContextData, updateContext } as any}
          >
            <ProfileContext.Provider
              value={{ profileDataContext, updateProfileContext } as any}
            >
