...
export default function DepositInvesment({ state }: { state: userDataStateType }) {
  ...
  async function handleSubmit(ev: any) {
    ...
    if (data.paymentMethod === "account") {
      ...
      try {
        const reqAcctBal = await auth.getAccountBalance(cookies['xat'], { mode: "normal", amount: data.amount });
        ...
        console.log(error)
        setLoadingState(false)
        showAlert("error", error.response.data?.description.desc ||  error.response.data)
      }
      return;
